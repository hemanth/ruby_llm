...
- **Provider tools use `with_provider_tools`.** Release-candidate users should rename `with_server_tools`, the `server_tools` reader and Agent macro, and the hosted-research `server_tools:` keyword to their `provider_tools` equivalents. Arguments and behavior are unchanged.
